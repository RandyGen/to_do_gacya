...
class MonthlyTodoListStore {
  /// 保存時のキー
  final String _saveKey = "MonthlyTodo";

  /// Todoリスト
  List<MonthlyTodo> _list = [];

  /// ストアのインスタンス
  static final MonthlyTodoListStore _instance =
      MonthlyTodoListStore._internal();
  ...
  MonthlyTodo findByIndex(int index) {
    return _list[index];
  }
  ...
  void add(bool done, String title, String detail) {
    ...
    var todo = MonthlyTodo(id, title, detail, done, dateTime, dateTime);
    _list.add(todo);
    save();
  }

  /// Todoを更新する
  void update(MonthlyTodo todo, bool done, [String? title, String? detail]) {
    todo.done = done;
    if (title != null) {
      todo.title = title;
    }
  ...
  void load() async {
    ...
    _list = loadTargetList
        .map((a) => MonthlyTodo.fromJson(json.decode(a)))
        .toList();
  }
}
