...
import 'daily_todo_list_store.dart';
...
/// 以下の責務を持つ
/// ・Todo入力画面の状態を生成する
class DailyTodoInputPage extends StatefulWidget {
  /// Todoのモデル
  final DailyTodo? todo;

  /// コンストラクタ
  /// Todoを引数で受け取った場合は更新、受け取らない場合は追加画面となる
  const DailyTodoInputPage({Key? key, this.todo}) : super(key: key);

  /// Todo入力画面の状態を生成する
  @override
  State<DailyTodoInputPage> createState() => _DailyTodoInputPageState();
}
...
/// ・Todoを追加/更新する
/// ・Todoリスト画面へ戻る
class _DailyTodoInputPageState extends State<DailyTodoInputPage> {
  /// ストア
  final DailyTodoListStore _store = DailyTodoListStore();
