...
class DailyTodoListStore {
  /// 保存時のキー
  final String _saveKey = "DailyTodo";

  /// Todoリスト
  List<DailyTodo> _list = [];

  /// ストアのインスタンス
  static final DailyTodoListStore _instance = DailyTodoListStore._internal();

  /// プライベートコンストラクタ
  DailyTodoListStore._internal();

  /// ファクトリーコンストラクタ
  /// (インスタンスを生成しないコンストラクタのため、自分でインスタンスを生成する)
  factory DailyTodoListStore() {
    return _instance;
  }
  ...
  DailyTodo findByIndex(int index) {
    return _list[index];
  }
  ...
  void add(bool done, String title, String detail) {
    ...
    var todo = DailyTodo(id, title, detail, done, dateTime, dateTime);
    _list.add(todo);
    save();
  }

  /// Todoを更新する
  void update(DailyTodo todo, bool done, [String? title, String? detail]) {
    todo.done = done;
    if (title != null) {
      todo.title = title;
    }
  ...
  void load() async {
    ...
    _list =
        loadTargetList.map((a) => DailyTodo.fromJson(json.decode(a))).toList();
  }
}
