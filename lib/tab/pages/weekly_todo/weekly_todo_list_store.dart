...
class WeeklyTodoListStore {
  /// 保存時のキー
  final String _saveKey = "WeeklyTodo";

  /// Todoリスト
  List<WeeklyTodo> _list = [];

  /// ストアのインスタンス
  static final WeeklyTodoListStore _instance = WeeklyTodoListStore._internal();

  /// プライベートコンストラクタ
  WeeklyTodoListStore._internal();

  /// ファクトリーコンストラクタ
  /// (インスタンスを生成しないコンストラクタのため、自分でインスタンスを生成する)
  factory WeeklyTodoListStore() {
    return _instance;
  }
  ...
  WeeklyTodo findByIndex(int index) {
    return _list[index];
  }
  ...
  void add(bool done, String title, String detail) {
    ...
    var todo = WeeklyTodo(id, title, detail, done, dateTime, dateTime);
    _list.add(todo);
    save();
  }

  /// Todoを更新する
  void update(WeeklyTodo todo, bool done, [String? title, String? detail]) {
    todo.done = done;
    if (title != null) {
      todo.title = title;
    }
  ...
  void load() async {
    ...
    _list =
        loadTargetList.map((a) => WeeklyTodo.fromJson(json.decode(a))).toList();
  }
}
